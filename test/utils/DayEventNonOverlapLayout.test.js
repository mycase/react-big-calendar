import { getStyledEvents } from '../../src/utils/DayEventNonOverlapLayout'
...
describe('getStyledEvents', () => {
  ...
  const slotMetrics = getSlotMetrics({ min, max, step: 30, timeslots: 2 })
  const accessors = { start: e => e.start, end: e => e.end }

  describe('Non overlapping algorithm', () => {
    ...
    const toCheck = [
      ...
      [
        'two consecutive events close together',
      ...
      [
        'one big event overlapping with two consecutive events starting close together',
        [
          { start: d(11), end: d(12) },
